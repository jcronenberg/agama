...
fn tun_config_to_dbus(config: &TunConfig) -> HashMap<&str, zvariant::Value> {
    ...
    tun_config.insert("multi-queue", config.multi_queue.into());
    tun_config.insert("pi", config.pi.into());
    tun_config.insert("vnet-hdr", config.vnet_hdr.into());
...
fn tun_config_from_dbus(conn: &OwnedNestedHash) -> Option<TunConfig> {
    ...
    if let Some(multi_queue) = tun.get("multi-queue") {
        tun_config.multi_queue = *multi_queue.downcast_ref::<bool>()?;
    }

    if let Some(pi) = tun.get("pi") {
        tun_config.pi = *pi.downcast_ref::<bool>()?;
    }

    if let Some(vnet_hdr) = tun.get("vnet-hdr") {
        tun_config.vnet_hdr = *vnet_hdr.downcast_ref::<bool>()?;
    }
